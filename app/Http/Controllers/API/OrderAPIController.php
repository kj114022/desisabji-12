...
class OrderAPIController extends Controller
{
    ...
    {
           Log::info("OrderAPIController :: show");
        /** @var Order $order */
    ...
    {
         Log::info("OrderAPIController :: store");
        ...
        try {
            ...
            if ($stripeToken->created > 0) {
                if (empty($input['delivery_address_id'])) {
                    ...
                    $order = $this->orderRepository->create(
                        $request->only('user_id', 'order_status_id', 'tax', 'delivery_address_id', 'delivery_fee', 'hint','total_price','total_discount','market_id')
                    );
                }
               ...
               $this->orderRepository->update(['payment_id' => $payment->id,'code' => $orderNo,'delivery_date'=> $deliveryDate], $order->id);
               $this->sendOrderNotifcation($user->mobile,$orderNo,$amountWithTax,$deliveryDate);
    ...
    private function cashPayment(Request $request)
    {
        ...
        try {
           ...
           foreach ($input['products'] as $productOrder) {
            ...
            Notification::send($order->market->users, new NewOrder($order));
    ...
    private function sendOrderNotifcation($userNumber,$orderNo,$orderAmount,$deliveryDate){

        $message = "Thank you for placing order number ".$orderNo.". You may expect delivery by ".$deliveryDate.". Amount to be paid on delivery Rs. ".$orderAmount.". Please check your email for more details.";
        $smsController = new SMSAPIController();
        $smsController->orderConfirmationSms($userNumber,$message);
    }
}
