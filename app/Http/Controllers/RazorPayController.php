...
class RazorPayController extends ParentOrderController
{
    ...
    {
          Log::info("OrderAPIController :: getRazorPayFields::".$this->order->user->cart[0]->product);
        $market = $this->order->user->cart[0]->product_city->market;
         Log::info("OrderAPIController :: market ::".  $market->name);
        $fields = array(
            'key_id' => config('services.razorpay.key', ''),
            'order_id' => $razorPayOrder['id'],
            'name' => $market->name,
            'description' => count($this->order->user->cart) ." items",
            'image' => $market->getFirstMedia('image')->getUrl('thumb'),
            'prefill' => [
                'name' => $user->name,
                'email' => $user->email,
